...
generating_embed = discord.Embed(
    ...
    description="Gambar sedang dibuat. Ini memerlukan waktu sekitar 30 hingga 50 detik.",
...
class RetryButton(ui.View):
    ...
    @discord.ui.button(label='Coba Lagi', emoji="♻️", style=discord.ButtonStyle.green, custom_id='retry')
    async def retry(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if image:
            ...
            await interaction.response.edit_message(content="Gagal mengunggah gambar.", embed=None)
            
    async def interaction_check(self, interaction):
        if interaction.user.id != self.p1:
            await interaction.response.send_message(f"Anda tidak memiliki akses untuk perintah ini, {interaction.user.mention}", ephemeral=True)
...
async def check_words(prompt: str) -> bool:
    banned_patterns = [
        ...
        r"\bsmall girl\b",
        r"\bsmall boy\b",
        r"\b(1[0-7]|[1-9])\s?yo\b",
        r"\b(1[0-7]|[1-9])\s?years?\s?old\b",
        r"\b(1[0-7]|[1-9])\s?-year-old\b",
    ]
...
async def update_stats():
    while True:
        ...
        activity_text = f"Di {len(client.guilds)} server | Melayani {total_members} anggota"
...
async def on_message(message):
    ...
    if message.channel.type == discord.ChannelType.private:
        title = 'Bot Hanya Bisa Digunakan di Server'
    else:
        title = 'Bot Sekarang Menggunakan Perintah / Saja'
...
@tree.command(name="stats", description="Dapatkan statistik bot")
async def stats(interaction: discord.Interaction):
    ...
    uptime_string = f"{days} hari, {int((total_seconds % 86400) // 3600)} jam, {int((total_seconds % 3600) // 60)} menit, {int(total_seconds % 60)} detik, dan {int(microseconds)} mikrodetik"
    embed = discord.Embed(
        title="Statistik Bot",
        description=f"Berikut adalah statistik bot saat ini:\n\nTotal Server: {total_servers}\nTotal Anggota: {total_members}\nWaktu Aktif: {uptime_string}",
        color=discord.Color.blue()
    )
    await interaction.response.send_message(embed=embed)

@client.command(name="machine", description="Dapatkan statistik mesin bot")
...
async def on_guild_join(guild):
    ...
    if owner:
        ...
        message_content = f"{owner_name} (ID: {owner_id}) menambahkan bot ke server mereka {server_name}."

        user = await client.fetch_user(1218584914405101659)
...
@tree.command(description="Generate gambar menggunakan AI")
@app_commands.describe(prompt="Deskripsi gambar", private="Apakah gambar ini hanya untuk Anda atau semua orang", image_type="Tipe gambar seperti anime, 3D, dll...")
@app_commands.choices(
    private = [
        app_commands.Choice(name="Ya", value="True"),
        app_commands.Choice(name="Tidak", value="False"),
    ],
    image_type = [
        app_commands.Choice(name="default・realistik", value="Realistic_Vision_v5.0"),
...
async def imagine(ctx: discord.Interaction, prompt: str, image_type: app_commands.Choice[str] = None, private: app_commands.Choice[str] = None):
    ...
    if await check_words(prompt):
        return await ctx.edit_original_response(content="Prompt Anda mengandung kata yang dilarang", embed=None)
